...
<head>
    ...
    <style>
        ...
        .telemetry-core { display: flex; gap: 15px; margin-bottom: 15px; }
        .car-container { display: flex; align-items: center; justify-content: center; gap: 8px; flex: 1; background: #0f0f0f; border: 1px solid var(--border); padding: 15px; border-radius: 4px; }
        .car-body { width: 50px; height: 120px; background: #222; border: 2px solid #555; border-radius: 20px; display: flex; align-items: center; justify-content: center; font-family: 'Bebas Neue'; color: #666; font-size: 1.5rem; letter-spacing: 2px; writing-mode: vertical-rl; text-orientation: upright;}
        .sonar-block { font-family: 'Bebas Neue'; font-size: 1.2rem; color: var(--success); padding: 5px 2px; border-radius: 4px; background: rgba(56, 142, 60, 0.1); border: 1px solid var(--success); text-align: center; width: 45px;}
        ...
        .ml-data-col { flex: 1; display: flex; flex-direction: column; gap: 10px; }
        .ml-box { flex: 1; background-color: #1a1a1a; padding: 10px; border-radius: 4px; border: 1px solid var(--border); display: flex; flex-direction: column; justify-content: center; align-items: center; font-family: 'Bebas Neue'; font-size: 1.5rem; }
        .ml-box .label { font-size: 0.9rem; color: #888; font-family: 'Roboto', sans-serif; display: block; margin-bottom: 5px;}
        .ml-box .status { color: var(--success); }
        .ml-box.alert-human .status { color: var(--warning); animation: blink 0.5s infinite; }
        .ml-box.alert-fire .status { color: var(--danger); animation: blink 0.5s infinite; }
...
<body>
            ...
            <div class="panel">
                ...
                <div class="telemetry-core">
                    ...
                    <div class="ml-data-col">
                        <div id="ml-human-box" class="ml-box">
                            <span class="label">ML HUMAN DETECTION</span>
                            <span class="status" id="human-val">CLEAR</span>
                        </div>
                        <div id="ml-fire-box" class="ml-box">
                            <span class="label">ML FIRE DETECTION</span>
                            <span class="status" id="ml-fire-val">CLEAR</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        window.onload = () => {
            ...
            // Force the dashboard to pull the AI-processed video from your laptop
        ...
        function toggleMode() {
            ...
            if(currentMode === 'MANUAL') {
                ...
                addLog("SYSTEM: Switched to AUTO Mode. Manual driving locked.");
            } else {
                currentMode = 'MANUAL';
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (key === 'm') { if(activeKey !== key) { toggleMode(); activeKey = key; } return; }
            if (key === 'p') { 
                if (!pumpActive) { pumpActive = true; sendCmd("P:ON"); document.getElementById('pump-ui').className = "status-box pump-on"; document.getElementById('pump-text').innerText = "ON"; } 
                return; 
            } 
            ...
            if (['w', 'a', 's', 'd'].includes(key)) {
                if (currentMode === 'AUTO') {
                    if (activeKey !== key) { triggerErrorBuzzer(); addLog("ACCESS DENIED: Switch to MANUAL mode to drive.", true); }
                    activeKey = key; return; 
                }
        ...
        document.addEventListener('keyup', (e) => {
            ...
            if (key === 'p') { pumpActive = false; sendCmd("P:OFF"); document.getElementById('pump-ui').className = "status-box pump-off"; document.getElementById('pump-text').innerText = "OFF"; return; }
            ...
            if (['w', 'a', 's', 'd'].includes(key)) { if (currentMode === 'MANUAL') { sendCmd('M:S'); } if (activeKey === key) activeKey = null; }
        });

        function startServoMove() { if (servoInterval) clearInterval(servoInterval); stepServo(); servoInterval = setInterval(stepServo, 200); }
        function stopServoMove() { if (servoInterval) clearInterval(servoInterval); servoInterval = null; }
        function stepServo() {
        ...
        function updateServo(val) { currentAngle = parseInt(val); sendCmd("S:" + currentAngle); }

        function parseIncomingData(dataString) {
            // ML Fire
            if (dataString === "ALERT:FIRE") { 
                document.getElementById('ml-fire-val').innerText = "DETECTED"; document.getElementById('ml-fire-box').classList.add('alert-fire'); 
                alarms.fire = true; evaluateAlarms(); addLog("ML ALERT: Fire Detected!", true); 
            }
            else if (dataString === "CLEAR:FIRE") { 
                document.getElementById('ml-fire-val').innerText = "CLEAR"; document.getElementById('ml-fire-box').classList.remove('alert-fire'); 
                alarms.fire = false; evaluateAlarms(); 
            }
            // ML Human
            else if (dataString === "ALERT:HUMAN") { 
                document.getElementById('human-val').innerText = "DETECTED"; document.getElementById('ml-human-box').classList.add('alert-human'); 
                alarms.human = true; evaluateAlarms(); addLog("ML ALERT: Human Detected!", true);
            }
            else if (dataString === "CLEAR:HUMAN") { 
                document.getElementById('human-val').innerText = "CLEAR"; document.getElementById('ml-human-box').classList.remove('alert-human'); 
            ...
            // Fixed Split Telemetry Parsing
                ...
                if (gasLevel > 300) { gasEl.style.color = "var(--danger)"; if(!alarms.gas) { alarms.gas = true; evaluateAlarms(); addLog("GAS WARNING: High levels detected!", true); } } 
                else { gasEl.style.color = "var(--success)"; if(alarms.gas) { alarms.gas = false; evaluateAlarms(); } }
            }
                ...
                if (flameVals.length === 3) {
                    ...
                    if (flameVals[0] == "0") { fL.classList.add('alert-bg'); fL.innerHTML = "<span class='flame-label' style='color:#fff;'>LEFT FLAME</span>FIRE"; hardwareFireActive = true; } 
                    ...
                    if (flameVals[1] == "0") { fF.classList.add('alert-bg'); fF.innerHTML = "<span class='flame-label' style='color:#fff;'>FRONT FLAME</span>FIRE"; hardwareFireActive = true; } 
                    ...
                    if (flameVals[2] == "0") { fR.classList.add('alert-bg'); fR.innerHTML = "<span class='flame-label' style='color:#fff;'>RIGHT FLAME</span>FIRE"; hardwareFireActive = true; } 
                    else { fR.classList.remove('alert-bg'); fR.innerHTML = "<span class='flame-label'>RIGHT FLAME</span>CLEAR"; }
                    
                    if (hardwareFireActive && !alarms.fire) { alarms.fire = true; evaluateAlarms(); addLog("HARDWARE ALERT: Fire Detected!", true); } 
                    else if (!hardwareFireActive && alarms.fire && document.getElementById('ml-fire-val').innerText === "CLEAR") { alarms.fire = false; evaluateAlarms(); }
                }
            }
        }
    </script>
...
// ----------------------------------------------------
// THE BUG FIX IS HERE: Safe Memory String Construction
// ----------------------------------------------------
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
    if (type == WStype_TEXT)
    {
        ...
        // Build the string safely, byte by byte, ignoring garbage memory
        String msg = "";
        ...
        {
            webSocket.broadcastTXT(msg); // Relays the clean Python ML commands directly to the browser
        }
    }
}
